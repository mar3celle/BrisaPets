        ...
        function renderAppointments() {
            ...
            if (upcoming.length === 0) {
                 list.innerHTML = `<p class="text-gray-500 italic p-4 bg-gray-50 rounded-lg">Sem agendamentos futuros registados.</p>`;
                 return;
            }

            upcoming.forEach(appt => {
                const isNew = appt.status === 'Novo';
                const statusColor = isNew ? 'bg-red-500' : 'bg-green-500';
                const statusText = isNew ? 'text-red-700 bg-red-100' : 'text-green-700 bg-green-100';

                const item = document.createElement('div');
                item.className = 'flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 rounded-lg border-l-4 border-primary-bp shadow-sm bg-white';
                item.innerHTML = `
                    <div class="flex-1 min-w-0">
                        <div class="font-bold text-gray-800">${appt.time} - ${appt.pet} (${appt.tutor})</div>
                        <div class="text-sm text-gray-600 truncate">${appt.service} | ${new Date(appt.date).toLocaleDateString('pt-PT', {day: '2-digit', month: 'short'})}</div>
                    </div>
                    <div class="flex items-center space-x-3 mt-2 sm:mt-0">
                        <span class="px-2 py-0.5 text-xs font-semibold rounded-full ${statusText}">${appt.status}</span>
                        ${isNew ? `<button onclick="confirmAppointment(${appt.id})" class="bg-green-500 hover:bg-green-600 text-white text-xs py-1 px-3 rounded transition duration-200">Confirmar</button>` : ''}
                    </div>
                `;
        ...
        function renderStats() {
            document.getElementById('stat-appointments').textContent = mockStats.totalAppointments;
            document.getElementById('stat-revenue').textContent = `€ ${mockStats.totalRevenue.toFixed(2)}`;
            document.getElementById('stat-sitting-revenue').textContent = `€ ${mockStats.sittingRevenue.toFixed(2)}`;
        }
        ...
        function initSitting() {
            renderSittingSessions('ativo');
            document.getElementById('sitting-filter').addEventListener('change', (e) => {
        ...
        function renderSittingSessions(filter) {
            ...
            if (filteredSessions.length === 0) {
                list.innerHTML = `<p class="text-gray-500 italic p-4 bg-gray-50 rounded-lg">Nenhuma sessão ${filter} encontrada.</p>`;
                return;
            }
            ...
            filteredSessions.forEach(session => {
                ...
                sessionElement.innerHTML = `
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 border-b pb-3">
                        <div>
                            ...
                            <h3 class="text-xl font-bold text-gray-800 mt-1">${session.pet} (<span class="font-normal text-primary-bp">${session.tutor}</span>)</h3>
                            <p class="text-sm text-gray-500"><i class="fas fa-calendar-alt mr-1"></i> Período: ${session.period}</p>
                        </div>
                        <div class="text-sm font-medium mt-3 sm:mt-0">${session.photoCount} Fotos</div>
                    </div>

                    <!-- Galeria de Fotos -->
                    <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3 mb-4" id="gallery-${session.id}">
                        <!-- Fotos serão adicionadas aqui -->
                    </div>

                    <button onclick="deletePhotos(${session.id})" class="bg-red-500 hover:bg-red-600 text-white text-sm py-2 px-4 rounded transition duration-200">
                ...
                session.photos.forEach(photoName => {
                     gallery.innerHTML += `<img src="https://placehold.co/150x150/ffb1c3/333?text=Foto" alt="Foto do Pet" class="w-full h-auto rounded-lg object-cover">`;
                });
            });
        }
        ...
        function initAppointmentForm() {
            renderCalendar();
            document.getElementById('service-type').addEventListener('change', toggleServiceType);
            document.getElementById('appointment-details-form').addEventListener('submit', handleAppointmentSubmit);
        }

        function toggleServiceType(event) {
            const type = event.target.value;
            document.getElementById('time-slot-container').classList.toggle('hidden', type === 'sitting');
            document.getElementById('period-container').classList.toggle('hidden', type !== 'sitting');
        }
        ...
        function renderCalendar() {
            ...
            calendarGrid.querySelectorAll('.day').forEach(d => d.remove()); // Remove dias antigos
            ...
            for (let i = 0; i < padding; i++) {
                calendarGrid.innerHTML += `<div class="calendar-day-box day day-disabled"></div>`;
            }
        ...
        function searchClient() {
            ...
            if (results.length > 0) {
                ...
                results.forEach(user => {
                 ...
                 resultsDiv.classList.remove('hidden');
                 resultsDiv.innerHTML = `<div class="p-2 text-sm text-gray-500 italic">Nenhum cliente/pet encontrado.</div>`;
            }
        }

        function selectClient(user) {
            document.getElementById('pet-search').value = user.name;
        ...
        function handleAppointmentSubmit(event) {
            ...
            const petSearch = document.getElementById('pet-search').value;
